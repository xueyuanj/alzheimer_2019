...
# match batch design and cluster information
clusterinfor = trem2_seurat_filter$seurat_clusters
...
ad_cells = table(clusterinfor[ad])
trem2ad_cells = table(clusterinfor[trem2ad])
wt_cells = table(clusterinfor[wt])
trem2ko_cells = table(clusterinfor[trem2ko])

raw_table=rbind(ad_cells, trem2ad_cells, wt_cells, trem2ko_cells)

# percentage of cluster per batch
per_batch = raw_table/rowSums(raw_table)
colnames(per_batch) = new_cluster_ids
per_batch

# percentage of batch per cluster
per_cluster = t(raw_table)/colSums(raw_table)
rownames(per_cluster) = new_cluster_ids
per_cluster

# plot heatmap
trem2.markers = FindAllMarkers(trem2_seurat_filter, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
top10 = trem2.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DoHeatmap(trem2_seurat_filter, features = top10$gene) + NoLegend()
...
#cluster_df = as.data.frame(clusterinfor)
...
plotbox = function(x){
  ...
  boxplot(wt_set[,x],trem2ko_set[,x], trem2ad_set[, x], ad_set[,x], notch=T, 
          outline=F, names= c("WT", "Trem2-/-", "Trem2-/-5XFAD", "Trem2+/+5XFAD"),
          main = x)
}
